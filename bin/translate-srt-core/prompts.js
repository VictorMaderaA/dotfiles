  ...
  const glossarySection = meta.glossary.length
    ? `\nGLOSARIO OBLIGATORIO — respeta estos términos en toda la traducción:\n` +
      meta.glossary
        .map(
          (t) =>
            ` • "${t.original}" → "${t.spanish}"${t.note ? ` (${t.note})` : ""}`,
        )
        .join("\n") +
      "\n"
    : "";
...
2. Corrige errores de transcripción ASR (Whisper / AssemblyAI): palabras partidas o fusionadas, homófonos incorrectos, puntuación ausente, alucinaciones. Esto NO incluye frases cortadas por segmentación temporal (ver regla 16).
...
16. CADA ÍNDICE ES AHORA UNA UNIDAD DE SENTIDO COMPLETA (una oración, o un grupo de oraciones cuando la segmentación automática del audio no encontró una frontera clara de frase) — ya no un fragmento cronológico arbitrario. DENTRO de un mismo índice tienes libertad total para reordenar, fusionar cláusulas o reformular con naturalidad, como en cualquier traducción normal. Lo que sigue estando PROHIBIDO es trasladar, fusionar o adelantar contenido de un índice a OTRO índice distinto: cada índice se traduce de forma autocontenida; el bloque <CONTEXTO> es solo referencia de continuidad (género, tono, términos), nunca origen de contenido a incluir en la traducción de otro índice.
...
function buildUserPrompt(batch, context) {
  ...
  const inputBlock = batch
    .map((s) => `${s.index}: ${s.protectedText}`)
    .join("\n");
